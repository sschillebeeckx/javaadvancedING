...
public class FilePersonRepository implements PersonRepository {
    ...
    public Person findPersonById(int id) throws PersonNotFoundException {
        try {this.readFile();} catch (IOException e) {System.out.println(e.getMessage());}
    ...
    public Person findPerson(String email, String password) throws PersonNotFoundException {
        try {this.readFile();} catch (IOException e) {System.out.println(e.getMessage());}
        return allPersons.stream().filter(p->p.getEmail().equals(email)&&p.getPassword().equals(password)).findFirst().orElseThrow(() -> new PersonNotFoundException("Login not correct. Try again."));
    }
