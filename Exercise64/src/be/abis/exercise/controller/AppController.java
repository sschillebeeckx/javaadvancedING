...
import be.abis.exercise.gui.Registration;
import be.abis.exercise.gui.RegistrationOK;
...
public class AppController {
    ...
    public void registerAction(Registration registration, Person p) {
        try {
            pr.addPerson(p);
            RegistrationOK ok= new RegistrationOK(p);
            registration.setVisible(false);
            ok.setVisible(true);
        } catch (IOException | PersonAlreadyExistsException e) {
            registration.fillErrorLabel(e.getMessage());
            exceptionLogger.error(e.getMessage());
        }
    }
}
